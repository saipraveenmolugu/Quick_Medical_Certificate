...
export default function CertificateDetailClient({ certificateId }: CertificateDetailClientProps) {
    console.log("CertificateDetailClient: certificateId =", certificateId);
    const certificate = CERTIFICATE_TYPES.find((c) => c.id === certificateId);
    console.log("CertificateDetailClient: found certificate =", certificate?.name);

    if (!certificate) {
        console.log("CertificateDetailClient: certificate not found, triggering notFound()");
        notFound();
    }
